...
namespace InventorySystemCsharp
{
    public partial class Manager_home : Form
    {
        ...
        {
            if(model.Text!="" && type1.Text!="" && comboBox1.Text!="" && price.Text!="" && crewamount.Text != "")
            {
                try
                {
                    MySqlConnection conn = new MySqlConnection(@"datasource=127.0.0.1;port=3306;SslMode=none;username=root;password=;database=highlanderdb;");
                    string query = "insert into `contractlist`(`model`,`type1`,`materials`,`price`,`crewamount`) values('" + model.Text.Trim() + "','" + type1.Text.Trim() + "','" + comboBox1.Text.Trim() + "','" + price.Text.Trim() + "','" + crewamount.Text.Trim() + "')";
        ...
        {
            MySqlConnection conn = new MySqlConnection(@"datasource=127.0.0.1;port=3306;SslMode=none;username=root;password=;database=highlanderdb;");
            MySqlDataAdapter sda = new MySqlDataAdapter("select * from contractlist ", conn);
        ...
        private void itemlist_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if(e.RowIndex >= 0)
            {
                ...
                type1.Text = row.Cells[2].Value.ToString();
                comboBox1.Text = row.Cells[3].Value.ToString();
                price.Text = row.Cells[4].Value.ToString();
                crewamount.Text = row.Cells[5].Value.ToString();
            }
        }
        ...
        private void u_itemBtn_Click(object sender, EventArgs e)
        {
            if (u_modelTxt.Text != "" && u_partTxt.Text != "" && u_typeCombo.Text != "" && u_priceTxt.Text != "" && u_stockTxt.Text != "")
            {
                ...
                {
                    MySqlConnection conn = new MySqlConnection(@"datasource=127.0.0.1;port=3306;SslMode=none;username=root;password=;database=highlanderdb;");
                    string query = "update `contractlist` set `model`= '" + u_modelTxt.Text + "',`type1`= '" + u_partTxt.Text + "',`materials`= '" + u_typeCombo.Text + "',`price`='" + u_priceTxt.Text + "', `crewamount`= '" + u_stockTxt.Text + "'where `id`= '"+ u_itemcodeTxt.Text +"' ";
                    MySqlCommand cmd = new MySqlCommand(query, conn);
                    conn.Open();
                    cmd.ExecuteNonQuery();
                    conn.Close();
                    MessageBox.Show("The item model are updated successfully!");
                    u_itemcodeTxt.Clear();
                    u_itemcodeTxt.Enabled = false;
                    u_itemcodeTxt.Text = "Id Auto Number";
                    u_modelTxt.Clear();
        ...
        {
            MySqlConnection conn = new MySqlConnection(@"datasource=127.0.0.1;port=3306;SslMode=none;username=root;password=;database=highlanderdb;");
            MySqlDataAdapter u_sda = new MySqlDataAdapter("select * from contractlist ", conn);
        ...
        private void del_item_btn_Click(object sender, EventArgs e)
        {
            if (d_modelTxt.Text != "" && d_partTxt.Text != "" && d_typeCombo.Text != "" && d_priceTxt.Text != "" && d_instockTxt.Text != "")
            {
                ...
                {
                    MySqlConnection conn = new MySqlConnection(@"datasource=127.0.0.1;port=3306;SslMode=none;username=root;password=;database=highlanderdb;");
                    string query = "delete from `contractlist` where `id`= '" + d_itemcodeTxt.Text + "' ";
                    MySqlCommand cmd = new MySqlCommand(query, conn);
                    conn.Open();
                    cmd.ExecuteNonQuery();
                    conn.Close();
        ...
        {
            MySqlConnection conn = new MySqlConnection(@"datasource=127.0.0.1;port=3306;SslMode=none;username=root;password=;database=highlanderdb;");
            MySqlDataAdapter d_sda = new MySqlDataAdapter("select * from contractlist ", conn);
